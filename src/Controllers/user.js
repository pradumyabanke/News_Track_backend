...
<<<<<<< HEAD
      ...
      if (roleAssign = await RollCreation.findOne({ role_name: role, })) {
...
<<<<<<< HEAD
    ...
    if (pendingApprovals.length > 0) {
      return res.status(200).send({
        status: true,
        msg: "News Paper Agency Login Successfully With pending approvals.",
        data: userExists,
      });
    } else {
      return res.status(200).send({
...
<<<<<<< HEAD
        ...
        let publication = await VendorModel.findById(postUserId);
        if (publication) {
          if (allPosts[i].author_name && allPosts[i].author_name.trim() !== '') {
            allPosts[i].username = allPosts[i].author_name;
          } else {
            allPosts[i].username = publication.publisher_name;
          }
        }
